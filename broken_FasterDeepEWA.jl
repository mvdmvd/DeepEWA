...
function init_EWA(;
    ...
    payoff=pris_payoff,
    β=Inf64)
    return Q₀, κ, α, δ, payoff, β # default params is best response dynamics
end
...
function update_EWA(
    ...
    for i in 1:2 # for both players 
        ...
        for a in 1:2 # update the attractions
            ...
            Qᵤ = ((1 - α) * Q[i][a] + (1 / (1 - (1 - α)(1 - κ))) * (δ + (1 - δ) * I) * Π) / Nₙ # EWA updating rule
...
function EWA_step!(Q, κ, α, δ, payoff, β)
    s, σ = mixed_strategy_draw(β, Q) # compute new mixed strat and draw action
    Qₙ = update_EWA(s, Q, κ, α, δ, payoff) # compute new attractions based off payoffs
    return Qₙ, s, σ
end
...
function run_EWA_pure(parameters; T=1000000, print=false)
    Q₀, κ, α, δ, payoff, β = parameters
    print ? println("Prior Q: $Q₀, prior N: $N₀") : nothing

    a₁ = 0
    a₂ = 0

    Qₜ, sₜ, σₜ = EWA_step!(Q₀, κ, α, δ, payoff, β) # first iter from priors
    ...
    for t in 1:T
        ...
        Qₜ, sₜ, σₜ = EWA_step!(Qₜ, κ, α, δ, payoff, β)
    ...
    return Qₜ, sₜ, σₜ, lim_chaos, NE_found, Nash
end
